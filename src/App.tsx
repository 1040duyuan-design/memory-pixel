...
import type { PixelCharacter, AutonomousAction, SceneState } from './types';
...
function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      // 初始随机行为
      setCurrentAction(randomNextAction(saved));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (isActionFinished(currentAction)) {
        setCurrentAction(randomNextAction(character));
      }
    }, 30000); // 每30秒检查一次
  ...
  function handleCharacterCreated(char: PixelCharacter) {
    setCharacter(char);
    setCurrentAction(randomNextAction(char));
  }
